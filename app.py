...
def play():
    ...
    max_prize = max((p['amount'] for p in prizes_data), default=0)

    conn.close()
    return render_template('play.html', users=users, winners=winners, prize_amounts=prize_amounts, total_prizes=total_prizes, total_prize_pool=total_prize_pool, max_prize=max_prize)
...
def select_user(user_id):
    ...
    if user and user['used'] == 0 and prizes_left > 0 and session.get('challenge_passed'):
...
def export_winners_csv():
    ...
    cw.writerow(['Name', 'Prize Amount (MMK)', 'Date Won'])
